...
(provide code-walker
         ...
         code-walker-special);805 linhas inicio
...
(define (code-walker-non-expanded code start end lastline)
  ...
  (displayln result)
  result)
(define (code-walker-special code start end lastline)
  (define start-line start)
  (define end-line end)
  #;(displayln start)
  #;(displayln end)
  #;(displayln (syntax-e code))
  #;(displayln "here")
  #;(displayln (syntax-e (cdr (cdr (cdr (syntax-e code))))))
  #;(displayln "here2")
  #;(displayln (syntax-e (car (syntax-e (cdr (cdr (cdr (syntax-e code))))))))
  (displayln " SPECIALL")
  (define result (get-syntax-aux-position (car (syntax-e (cdr (cdr (cdr (syntax-e code)))))) (+ 1 start-line) end-line lastline))
  (displayln "result")
  (displayln result)
  result)
  ...
  (define (get-syntax program start end)
    ...
    (cond [(null? source-aux)
          ...
          [(syntax? source-aux)
           ...
           #;(displayln (syntax-position source-aux))
...
(define (get-syntax-aux-position program start end lastline)
  (define stop? #f)
  (define source-stack (list))
  (define aux-result null)
  #;(displayln (syntax-source program))
  ...
  (define (get-syntax program start end)
    ...
    (define (get-next-compare source-aux source-stack)
      ...
      #;(displayln (and (pair? source-stack) (pair? (car source-stack)) (syntax? (car (car source-stack)))))
      (if (and (pair? source-stack) (pair? (car source-stack)) (syntax? (car (car source-stack))))
          (set! aux (syntax-position (car (car source-stack))))
          (when (or (null? source-stack) (and (syntax? (car source-stack)) (null? (syntax-e (car source-stack)))))
            (set! aux -1)))
      aux)
    (cond [(null? source-aux)
          ...
          [(pair? source-aux)
           ...
           (get-syntax source-aux start end)]
          [(syntax? source-aux)
           (define compare-aux (syntax-position source-aux))
           #;(display "----------------------  compare-aux ")
           #;(displayln compare-aux)
           #;(displayln (syntax-position source-aux))
           (define next-compare null)
